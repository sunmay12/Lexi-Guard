...
- 현재 데이터셋은 인공적으로 생성한(synthetic) 환각 데이터를 기반으로 구축되어, 실제 RAG 시스템에서 생성되는 다양한 답변에 대한 일반화 성능을 충분히 보장하지는 않는다.
- 모델은 법령 원문과 생성 답변만을 입력으로 사용하며, 질문까지 함께 고려하는 end-to-end 질의응답 평가 모델은 아니다.
- 모델의 최대 입력 길이가 제한되어 있어 긴 조문이나 여러 문서를 근거로 생성된 답변에서는 일부 정보가 잘릴 수 있다.
- Faithful 샘플의 수가 Not_Faithful 샘플보다 적어, 보다 균형 잡힌 데이터셋으로 확장할 필요가 있다.
- Lexi-Guard의 예측 결과는 법률 전문가의 판단을 대체하는 것이 아니라, 생성 답변의 원문 충실성을 확인하기 위한 보조 검증 도구로 활용하는 것이 적절하다.


## 향후 개선 방향

- 실제 법률 QA 및 RAG 시스템에서 생성된 답변을 활용한 평가 데이터셋 구축
- 긴 법령 문서에서도 안정적으로 판별할 수 있는 입력 방식 연구
- 숫자 변경 유형을 법적 의미가 있는 숫자 변경과 개정일자/조문번호 변경으로 세분화
- 원문의 의미는 유지하지만 표현만 다른 Faithful 답변 데이터 확장
- 실제 법률 상담 답변에서 발생하는 다양한 환각 유형 분석
- RAG 시스템에서 생성된 답변을 자동으로 검증하는 후처리 모듈로 적용
